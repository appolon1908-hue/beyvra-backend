...
from .utils import generate_hashed_key
...
class DashboardMetricsTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        test_user = User.objects.create_user(**test_user_details)
        #self.users_data = f'user1 - {user1.id}, user2 - {user2.id}, test_user - {test_user.id}'
    ...
    def test_caching_feature(self):
        ...
        filters_for_test = json.dumps(filters_dict)
        payload = {'start_date': '2011-01-01', 'end_date': '2025-09-15', 'categories_filters': filters_for_test}

        # Generate hashed key for tested metrics
        metrics_key = generate_hashed_key('dashboard_metrics', payload)

        # Remove old cache(if exists) for metrics which will be tested
        ...
        metrics_cache = cache.get(metrics_key)
        cache.delete(metrics_key)
        self.assertTrue(metrics_cache, 'Metrics were not cached')
        self.assertDictEqual(json.loads(metrics_cache), expected_result, 'Cached metrics don\'t match with expected result')
    
    
    # Additional test for getting metrics using categories filters for all existing fields of every category
