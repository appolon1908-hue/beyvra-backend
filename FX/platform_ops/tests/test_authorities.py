...
from platform_ops.feature_flags.evaluator import HIGH_RISK,evaluate
...
class FailClosedAuthorityTests(SimpleTestCase):
    ...
    def test_reconciliation_zero_tolerance(self):self.assertEqual(summarize({})["state"],"PASS")
    def test_every_reconciliation_mismatch_fails(self):
        for check in CHECKS:self.assertEqual(summarize({check:1})["state"],"FAIL")
