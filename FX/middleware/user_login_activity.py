                ...
                try:
                    ...
                    if user:
                        ...
                        device_type = user_agent.split(
                            '/')[0] if user_agent else "Unknown"
                        device_model = user_agent.split(
                            '/')[1].split(' ')[0] if '/' in user_agent else "Unknown"
                        ...
                        with transaction.atomic():
                            UserActivity.objects.create(
                                user=user,
                                action_type='LOGIN',
                                action_status='SUCCESS' if status_code in [
                                    200, 201] else 'FAILED',
                                description="Login attempt" if status_code in [
                                    200, 201] else "Failed login attempt",
