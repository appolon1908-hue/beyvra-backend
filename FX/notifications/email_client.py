...
class EmailMiddlewareClient:
        ...
        with self._lock:
            ...
            try:
                response = requests.post(
                    ...
                    data={"grant_type": "client_credentials", "client_id": "beyvra-email-production",
                          "client_secret": self._credential(), "scope": "email.send email.read"},
    ...
    def submit(self, item, parameters: dict) -> dict:
        ...
        body = {"notification_id": str(item.notification_id), "event_id": item.event_id,
                "correlation_id": str(item.correlation_id), "idempotency_key": item.idempotency_key,
                "user_id": item.user_id_ref, "account_id": item.account_id_ref,
                "template_id": normalize_template(item.template_key), "template_version": int(item.template_version),
                "recipient": item.recipient_email, "event_type": item.event_type, "category": category,
                "locale": item.locale, "parameters": parameters}
        try:
            response = requests.post(settings.BEYVRA_EMAIL_API_URL + "/v1/email/messages", json=body,
                                     headers={"Authorization": "Bearer " + self.token()}, timeout=10, allow_redirects=False)
...
def normalize_template(value: str) -> str:
    return {"email_otp": "account_verification", "welcome_email": "welcome"}.get(value, value)


def category_for(template_id: str) -> str:
    if template_id in {"account_verification", "email_otp", "welcome", "welcome_email", "password_reset", "password_changed", "email_changed", "account_locked", "account_unlocked"}:
        return "ACCOUNT"
    if template_id in {"new_login", "suspicious_login", "new_device", "two_factor_changed", "security_settings_changed", "api_key_created", "api_key_revoked"}:
        return "SECURITY"
