...
class RestrictionCollectionView(APIView):
    ...
    def post(self,request):
        ...
        try:restriction=add_restriction(profile,r_type,str(request.data.get("reason_code",""))[:64],"MANUAL",request.user,expires_at=expires_at)
        except ValueError as exc:return safe_error(str(exc))
...
class OverrideCollectionView(APIView):
    ...
    def post(self,request):
        ...
        expires_at=parse_expiration(request.data.get("expires_at",""))
        if request.data.get("expires_at") and not expires_at:return safe_error("INVALID_EXPIRATION")
        try:override=request_override(profile,str(request.data.get("control","")),str(request.data.get("new_state","")),reason,request.user,expires_at,str(request.data.get("evidence_ref",""))[:255])
        except ValueError as exc:return safe_error(str(exc))
