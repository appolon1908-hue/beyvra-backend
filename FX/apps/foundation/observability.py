...
EXECUTION_ROUTE_DECISIONS = Counter("beyvra_execution_route_decisions_total", "Execution route decisions", ("mode", "decision"))
EXECUTION_PROVIDER_HEALTH = Gauge("beyvra_execution_provider_health", "Execution provider health state", ("provider", "mode"))
EXECUTION_PROVIDER_FAILURES = Counter("beyvra_execution_provider_failures_total", "Execution provider failures", ("provider", "failure_category"))
EXECUTION_ROUTE_DURATION = Histogram("beyvra_execution_route_duration_seconds", "Execution route evaluation latency", ("mode",))
EXECUTION_SLIPPAGE_BPS = Histogram("beyvra_execution_slippage_bps", "Measured side-aware simulated or paper execution slippage", ("provider", "mode"))
EXECUTION_UNKNOWN_OUTCOMES = Counter("beyvra_execution_unknown_outcomes_total", "Ambiguous execution outcomes", ("provider", "mode"))
EXECUTION_ROUTE_REQUESTS = Counter("beyvra_execution_route_requests_total", "Execution routing requests", ("asset_class","order_type","result","mode"))
EXECUTION_ROUTE_CANDIDATES = Counter("beyvra_execution_route_candidates_total", "Generated route candidates", ("provider","asset_class","order_type","result","mode"))
EXECUTION_ROUTE_SELECTED = Counter("beyvra_execution_route_selected_total", "Selected execution routes", ("provider","venue_class","asset_class","order_type","mode"))
EXECUTION_ROUTE_REJECTIONS = Counter("beyvra_execution_route_rejections_total", "Rejected execution routes", ("provider","asset_class","order_type","result","mode"))
EXECUTION_PROVIDER_REQUESTS = Counter("beyvra_execution_provider_requests_total", "Paper/simulation provider requests", ("provider","result","mode"))
EXECUTION_PROVIDER_LATENCY = Histogram("beyvra_execution_provider_latency_seconds", "Paper/simulation provider latency", ("provider","mode"))
EXECUTION_PROVIDER_REJECTIONS = Counter("beyvra_execution_provider_rejections_total", "Provider rejections", ("provider","asset_class","order_type","mode"))
EXECUTION_PARTIAL_FILLS = Counter("beyvra_execution_partial_fills_total", "Partial execution fills", ("provider","asset_class","mode"))
EXECUTION_QUALITY_SLIPPAGE = Histogram("beyvra_execution_quality_slippage_bps", "Side-aware execution slippage", ("provider","asset_class","mode"))
EXECUTION_PRICE_IMPROVEMENT = Histogram("beyvra_execution_price_improvement_bps", "Side-aware price improvement", ("provider","asset_class","mode"))
EXECUTION_RECONCILIATION_VIOLATIONS = Counter("beyvra_execution_reconciliation_violations_total", "Execution reconciliation violations", ("result",))
EXECUTION_CIRCUIT_BREAKER = Gauge("beyvra_execution_circuit_breaker_state", "Execution circuit state: closed=0 half-open=1 open=2", ("provider","mode"))
MARKET_DURATION = Histogram("beyvra_market_provider_duration_seconds", "Market provider latency", ("provider",))
PROVIDER_UP = Gauge("beyvra_provider_up", "Provider connection availability", ("provider","data_type","asset_class","environment"))
PROVIDER_REQUESTS = Counter("beyvra_provider_requests_total", "Provider requests", ("provider","data_type","asset_class","environment"))
PROVIDER_FAILURES = Counter("beyvra_provider_failures_total", "Provider failures", ("provider","data_type","asset_class","environment"))
PROVIDER_RATE_LIMITED = Counter("beyvra_provider_rate_limited_total", "Provider rate-limit responses", ("provider","data_type","asset_class","environment"))
PROVIDER_LATENCY = Histogram("beyvra_provider_latency_seconds", "Provider latency", ("provider","data_type","asset_class","environment"))
PROVIDER_RECONNECTS = Counter("beyvra_provider_reconnects_total", "Provider reconnect attempts", ("provider","data_type","asset_class","environment"))
PROVIDER_SEQUENCE_GAPS = Counter("beyvra_provider_sequence_gaps_total", "Provider sequence gaps", ("provider","data_type","asset_class","environment"))
MARKET_DATA_AGE = Gauge("beyvra_market_authority_data_age_seconds", "Canonical market data age", ("provider","data_type","asset_class","environment"))
MARKET_DATA_STALE = Counter("beyvra_market_authority_stale_total", "Canonical stale decisions", ("provider","data_type","asset_class","environment"))
MARKET_SNAPSHOT_DISAGREEMENT = Counter("beyvra_market_snapshot_disagreement_total", "Snapshot and stream disagreements", ("provider","data_type","asset_class","environment"))
MARKET_FAILOVER = Counter("beyvra_market_failover_total", "Observable provider failovers", ("provider","data_type","asset_class","environment"))
