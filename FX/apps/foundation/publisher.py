...
def publish_batch(limit=100):
    ...
    if not rows:
        if OutboxEvent.objects.filter(
            state=OutboxEvent.State.DEAD_LETTER
        ).exists():
            # Exhausted work remains unresolved; do not clear the liveness alert.
            return 0
    ...
    for event, error in asyncio.run(_publish(rows)):
        mark_publish_result(
            event,
            error_code=error,
            maximum_attempts=getattr(settings, "OUTBOX_MAX_ATTEMPTS", 10),
        )
        if error:
            OUTBOX_FAILURES.labels("dependency").inc()
            OUTBOX_RETRIES.inc()
        else:
