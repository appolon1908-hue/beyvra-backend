...
def account_scope(request):
 context=TradingContext.from_request(request);return context.tenant_ref,context.account_ref
...
class CostBasis(Base):
 def get(self,r,instrument_id=None):
  tenant,account=account_scope(r);rows=CostBasisPosition.objects.filter(tenant_ref=tenant,account_ref=account); rows=rows.filter(instrument_id=instrument_id) if instrument_id else rows
...
class Lots(Base):
 def get(self,r,lot_id=None):
  tenant,account=account_scope(r);rows=TaxLot.objects.filter(tenant_ref=tenant,account_ref=account); rows=rows.filter(pk=lot_id) if lot_id else rows
...
class Pnl(Base):
 model=None
 def get(self,r,instrument_id=None):
  tenant,account=account_scope(r);rows=self.model.objects.filter(tenant_ref=tenant,account_ref=account); rows=rows.filter(instrument_id=instrument_id) if instrument_id else rows
...
class Nav(Base):
 def get(self,r):
  tenant,account=account_scope(r);rows=PortfolioNavSnapshot.objects.filter(tenant_ref=tenant,account_ref=account).order_by("-valuation_time")[:100]
