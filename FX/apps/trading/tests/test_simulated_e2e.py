...
class SimulatedTradingE2ETests(TestCase):
    ...
    def test_cancel_stale_expected_version_has_no_effect(self):
        ...
        semantic_conflict = self.cancel_order(order, "cancel-stale", order.updated_at.isoformat())
        self.assertEqual(semantic_conflict.status_code, 409)
        self.assertEqual(semantic_conflict.json()["error"]["code"], "IDEMPOTENCY_CONFLICT")
        order.refresh_from_db()
