...
def _create_allowed(user, data, idempotency_key, correlation_id=None, idempotency_record=None):
    ...
    if result.decision != "ALLOW":
        ...
        enqueue_event(aggregate_type="execution", aggregate_id=order.id, event_type="trading.execution.submitted.v1", payload=event_payload(order, execution_mode="SIMULATION"), tenant_ref=tenant, correlation_id=correlation)
...
def apply_execution(order_id, execution):
    ...
    def mutation():
        ...
        if order.filled_quantity:
            ...
            enqueue_event(aggregate_type="execution_quality", aggregate_id=quality.report_id, event_type="trading.execution.quality.updated.v1",
