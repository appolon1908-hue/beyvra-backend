...
class PostTradeExceptionService:
    ...
    @staticmethod
    def assign(row, *, actor_ref):
        row.assigned_to = actor_ref; row.state = "ASSIGNED"; row.save(update_fields=("assigned_to", "state", "updated_at")); return row

    @staticmethod
    def escalate(row, *, actor_ref):
        row.state = "ESCALATED"; row.save(update_fields=("state", "updated_at")); return row

    @staticmethod
    @transaction.atomic
    def resolve(row, *, actor_ref, resolution_code):
        ...
        audit(tenant_ref=row.tenant_ref, actor_ref=actor_ref, action="post_trade.exception.resolved", resource_type="post_trade_exception", resource_ref=row.id, evidence={"resolution_code": resolution_code})
