...
def create_account_type(currency="USD", name="US dollar", is_fiat=False, is_active=True):
    """Create and return account type."""
    pass
    # return AccountType.objects.create(
    #     currency=currency, name=name, is_fiat=is_fiat, is_active=is_active
    # )
...
class PrivateWalletsApiTests(TestCase):
    ...
    def test_wallets_limited_to_user(self):
        ...
        self.assertEqual(str(res.data[0]["user"]), str(self.user.id))
        self.assertEqual(str(res.data[0]["id"]), str(wallet.id))
    ...
    def test_create_wallet_inactive_account_type_fails(self):
        ...
        account_type = create_account_type(symbol="INR", name="Indian rupee", is_active=False)
        payload = {"account_type": str(account_type.id)}
        res = self.client.post(WALLETS_URL, payload)
