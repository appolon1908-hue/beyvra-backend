...
class BankAccountView(APIView):
    ...
    def post(self, request):
        ...
        try:
            bank_account_serializer = BankAccountSerializer(data=request.data, context={"request": request})
...
class WithdrawalRequestView(APIView):
    ...
    def get(self, request):
        ...
        if withdrawal_id:
            withdrawal_request_instance = WithdrawalRequest.objects.filter(withdrawal_id=withdrawal_id).first()
            return Response(
                {"data": WithdrawalRequestSerializer(withdrawal_request_instance).data}, status=status.HTTP_200_OK
            )
        else:
            ...
            return Response(
                {"data": WithdrawalRequestSerializer(withdrawal_request_instance, many=True).data},
    ...
    def post(self, request):
        try:
            bank_account = BankAccount.objects.filter(
                bank_name=request.data["bank_name"], account_number=request.data["account_number"]
            ).first()
            data = request.data
            if not bank_account:
                return Response(
                    {
                        "Error": "Bank account not found for account number {} in {} bank".format(
                            request.data["account_number"], request.data["bank_name"]
                        )
                    },
                    status=status.HTTP_404_NOT_FOUND,
                )
    ...
    def patch(self, request):
        try:
            ...
            withdrawal_instance = get_object_or_404(WithdrawalRequest, withdrawal_id=withdrawal_id)
...
class AdminBankAccountView(APIView):
    ...
    def get(self, request):
        ...
        return Response(
            {"data": BankAccountSerializer(bank_account_instance, many=True).data}, status=status.HTTP_200_OK
        )
