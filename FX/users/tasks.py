...
def _otp_delivery_is_current(item, now):
    ...
    return PendingRegistration.objects.filter(
        ...
        challenges__send_count=send_count,
    ).exists()
...
def process_transactional_email_outbox(self):
    ...
    try:
        ...
        if not _otp_delivery_is_current(item, timezone.now()):
            ...
            item.save(update_fields=["status", "last_error_code", "lease_expires_at"])
            logger.info(
                "transactional_email_stale_otp_suppressed",
                extra={"outbox_id": str(item.pk), "attempt_count": item.attempt_count},
            )
            return "failed"
        ...
        item.provider_message_id = str(
            result.get("provider_message_id") or result.get("notification_id") or ""
        )
        ...
        if item is not None:
            item.status = (
                "dead_letter" if not exc.retryable or item.attempt_count >= 5 else "pending"
            )
