...
Known-account login failures append safe `LOGIN_FAILURE` events without raw credentials, IP addresses, or user agents. Five failures within ten minutes elevate the event to `HIGH` with `TOO_MANY_FAILED_LOGINS`; unknown usernames do not create fabricated account evidence. Public validation errors remain identical and the anonymous throttle provides burst protection.
