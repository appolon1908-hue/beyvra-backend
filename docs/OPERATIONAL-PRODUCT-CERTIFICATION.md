...
- The complete backend suite now discovers 106 tests and passes 105 with one PostgreSQL-trigger assertion intentionally skipped under SQLite. The PostgreSQL 16 scoped suite independently passes that trigger coverage. Artifact-worker storage failures are bounded to three retries and atomically mark jobs `FAILED` rather than leaving them in `RUNNING`.
