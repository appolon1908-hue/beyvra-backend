...
- Python 3.11.15 and PostgreSQL 16.14: migration from zero, operations rollback, reapply, drift check, and system check pass. The current PostgreSQL scoped suite has 36 passing tests, including direct bulk-update/delete attempts against the audit trigger; the corresponding SQLite suite passes with the PostgreSQL-only trigger assertion skipped.
...
- The complete backend suite now discovers 99 tests and passes 98 with one PostgreSQL-trigger assertion intentionally skipped under SQLite. The PostgreSQL 16 scoped suite independently passes that trigger coverage. Artifact-worker storage failures are bounded to three retries and atomically mark jobs `FAILED` rather than leaving them in `RUNNING`.
